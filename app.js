...
const nodes = storyboard ? Array.from(storyboard.querySelectorAll('.diagram-node')) : [];
...
if (storyboard && stagePills.length && currentStage && currentTitle && currentCopy && currentBadge && currentBullets && pipelineDiagram && nodes.length) {
  let stageIndex = 0;
  ...
  const renderStage = () => {
    ...
    currentBullets.innerHTML = stage.bullets.map((bullet) => `<li>${bullet}</li>`).join('');
